...
## UUI shell checklist (Untitled + NGX tokens)

Use this when touching **`@/components/ui/*`**, **`app/globals.css`**, **`legacy-theme`**, **`uui-ngx-token-bridge`**, or **Design-System** primitives.

| Topic | Do |
|--------|-----|
| Brand / primary fills | Map `--color-brand-*` via **`--ds-*`** + bridge + **`[data-theme="legacy"]`**; avoid unlayered `:root` brand overrides that reference `--primary` in a cycle |
| `Button` | Prefer **`iconLeading` / `iconTrailing`**; `UuiAppButton` promotes icon siblings when safe; compound triggers (chevron + badge) should use explicit slots |
| `Select` | **`SelectValue`** includes **`data-slot="select-value"`**; trigger keeps **`w-full min-w-0`** |
| `DropdownMenuItem` | Default row is **`flex-row flex-nowrap`** + **`min-w-0`**; wrap **icon + label** in **`inline-flex items-center gap-2`** when mixing with extra nodes |
| `AccordionTrigger` | Title in **`min-w-0 flex-1`**; one **shrink-0** icon cell for expand/collapse |
| Dense toolbars | Parent **`flex`** rows: **`min-w-0`**, **`shrink-0`** on controls, **`whitespace-nowrap`** where spec disallows wrap |
| Verification | **`/dev/uui-studio/facades`** + one heavy page (e.g. RFIs) after CSS or shell edits |

---

## Primitive alignment (facade defaults)

**Full matrix + rules:** `docs/primitive-facade-defaults.md` (Untitled vs RAC vs UI-next per `components/ui` module; reference vs product shell; token/layer rules).

| Check | Rule |
|--------|------|
| Import surface | App code → **`@/components/ui/*`**. ESLint blocks **`@untitledui/react`** outside `components/ui`, `components/ui-next`, `Design-System`. |
| Toast API | **`import { toast, useToast } from "@/lib/toast"`** in app. **`@/hooks/*`** is Design-System-only — ESLint **`no-restricted-imports`** blocks `@/hooks/use-toast` under **`app/`**. |
| After CSS/token edits | **`pnpm run lint`** · **`pnpm run lint:compliance`** (when `app/` or `components/ui/` touched) · spot-check **`/dev/uui-studio/facades`**. |
| New facade / headless choice | Extend or add under **`components/ui/`**; document UUI vs RAC decision per **`docs/primitive-facade-defaults.md`**. |
...
| `bg-background-secondary` | Row hover, zebra, muted strips — **not** static headers (see `app/legacy-theme.css` §02) |
